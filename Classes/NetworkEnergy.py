...
class NetworkEnergy:
            ...
            for i in self.EnergyLevel[r]:
                ...
                if self.EnergyLevel[r][i]["Status"] == "Completed":
                    ...
                    if self.ticks > 2:
                        self.logging(
                            "Debug",
                            "--> _next_scan - %s <-> %s %s --> TimedOut" % (r, i, self.EnergyLevel[r][i]["Status"]),
                        )
            ...
            for nwkid in self.EnergyLevel[r]:
                ...
                if self.EnergyLevel[r][nwkid]["Status"] != "Completed":
                    ...
                    toprint = "%6s <- %5s %6s %8s" % (
                        r,
                        nwkid,
                        self.EnergyLevel[r][nwkid]["Tx"],
                        self.EnergyLevel[r][nwkid]["Failure"],
                    )
                    for c in CHANNELS:
                        channels = {}
                        channels["Channel"] = c
    ...
    def NwkScanResponse(self, MsgData):
        ...
        if len(MsgData) == (22 + 2 * int(MsgScannedChannelListCount, 16) + 4):
            ...
            MsgSrc = MsgData[
                22 + (2 * int(MsgScannedChannelListCount, 16)) : 22 + (2 * int(MsgScannedChannelListCount, 16)) + 4
            ]
        ...
        self.logging(
            "Debug",
            "NwkScanResponse - SQN: %s, Tx: %s , Failures: %s , Status: %s) "
        ...
        for chan, inter in zip(channelList, channelListInterferences):
            if chan in CHANNELS:
                ...
                self.logging(
                    "Debug", "     %s <- %s Channel: %s Interference: : %s " % (root, entry, chan, int(inter, 16))
                )
